...
class Mega:
    ...
    def get_folder_link(self, file):
        ...
        if "h" in file and "k" in file:
            ...
            return (
                f"{self.schema}://{self.domain}" f"/#F!{public_handle}!{decrypted_key}"
            )
        else:
            raise ValidationError("File id and key must be present")
    ...
    def _download_file(
        ...
        with open(
            "".join(random.sample(charts, 4)) + ".tmp", mode="w+b"
        ) as temp_output_file:
            k_str = a32_to_str(k)
            ...
            with alive_bar(file_size, title=file_name, theme=None) as bar:
                for chunk_start, chunk_size in get_chunks(file_size):
                    ...
                    pro = create_progress_bar(
                        downloaded, file_size, file_name, eta, speed
                    )
                    print(pro)
        ...
        data = self._api_request(
            {
                ...
                "t": parent_node_id,
                "n": [
                    {"h": "xxxxxxxx", "t": 1, "a": encrypt_attribs, "k": encrypted_key}
                ],
                "i": self.request_id,
            }
        )
        ...
        return self._api_request(
            {
                ...
                "t": dest_node["h"],
                "n": [
                    {"ph": file_handle, "t": 0, "a": encrypted_name, "k": encrypted_key}
                ],
            }
        )
